...
const tools = [
  ...
  ['vent','C','換氣量計算','依長寬高與ACH估算 CMH / CMM / CFM'], ['cool','C','冷負載估算','依長寬與模式初估容量'], ['air','C','風量估算','依冷負載與ΔT估算風量'],
...
function initDcSharedTool(){const calc=()=>{const vals=['rw','rr','rk','rl','rwid','rh','pp','uf','df','ld','or','vv','pf'].map(x=>+document.getElementById(x).value);if(!vals.every(Number.isFinite)){rrr.textContent='-';return;}const [rows,per,kw,l,w,h,p,u,dfv,ldv,otherR,v,pff]=vals;const m=unit.value==='cm'?0.01:1,L=l*m,W=w*m,H=h*m,area=L*W,vol=area*H,tr=rows*per,it=tr*kw,ups=it*u,dist=it*dfv,light=area*ldv/1000,people=p*0.1,totalHeatKw=it+ups+dist+light+people,itUpsSupply=it+ups,otherAuxPowerKw=it*otherR,totalHeatRt=totalHeatKw/RT_PER_KW,chillerKw=totalHeatRt*WATER_COOLED_HVAC_FACTORS.chiller,coolingTowerKw=totalHeatRt*WATER_COOLED_HVAC_FACTORS.coolingTower,chwPumpKw=totalHeatRt*WATER_COOLED_HVAC_FACTORS.chwPump,cwPumpKw=totalHeatRt*WATER_COOLED_HVAC_FACTORS.cwPump,ahuFanKw=totalHeatRt*WATER_COOLED_HVAC_FACTORS.ahuFan,totalCoolingPowerKw=chillerKw+coolingTowerKw+chwPumpKw+cwPumpKw+ahuFanKw,tp=itUpsSupply+totalCoolingPowerKw+otherAuxPowerKw,cur=(P)=>P*1000/(Math.sqrt(3)*v*pff),totalCurrentA=cur(tp),recommendedUpsKw=roundUpToStep(itUpsSupply,UPS_STEPS),recommendedCoolingRt=roundUpToMultiple(totalHeatRt,5),recommendedNfbA=roundUpToStep(totalCurrentA,NFB_STEPS),otherInfraKw=ups+dist+light+people+otherAuxPowerKw,facilityTotalKw=it+otherInfraKw+totalCoolingPowerKw,pue=it>0?facilityTotalKw/it:null,pueHint=pue===null?'':pue<1.3?'理論值偏高效，仍需以實測電表確認。':pue<1.6?'理論值落在常見資料中心初估區間，仍需以實測確認。':'理論值偏高，建議檢查空調系統效率、泵浦與風機估算。';rrr.innerHTML=`<section id='dc-space' class='dc-section'><h3>A. 機房空間</h3><table class='mobile-hide'><tr><td>排數</td><td>${format1(rows)}</td></tr><tr><td>每排機櫃數</td><td>${format1(per)}</td></tr><tr><td>總機櫃數</td><td>${format1(tr)}</td></tr><tr><td>每櫃 kW</td><td>${format1(kw)}</td></tr><tr><td>面積 m²</td><td>${format1(area)}</td></tr><tr><td>坪數</td><td>${format1(area/3.3058)}</td></tr><tr><td>體積 m³</td><td>${format1(vol)}</td></tr></table>${renderMobileResultCards(['項目','數值'], [['排數',format1(rows)],['每排機櫃數',format1(per)],['總機櫃數',format1(tr)],['每櫃 kW',format1(kw)],['面積 m²',format1(area)],['坪數',format1(area/3.3058)],['體積 m³',format1(vol)]])}</section><section id='dc-heat' class='dc-section'><h3>B. 散熱評估</h3><table class='mobile-hide'><tr><th>項目</th><th>kW</th><th>RT</th><th>BTU/h</th></tr>${[['IT設備',it],['UPS（UPS 損耗 / 發熱）',ups],['配電系統',dist],['照明設施',light],['人員',people],['合計',totalHeatKw]].map(([n,vv])=>`<tr><td>${n}</td><td>${format1(vv)}</td><td>${format1(vv/RT_PER_KW)}</td><td>${format1(vv*3412.142)}</td></tr>`).join('')}</table>${renderMobileResultCards(['項目','kW','RT','BTU/h'], [['IT設備',format1(it),format1(it/RT_PER_KW),format1(it*3412.142)],['UPS（UPS 損耗 / 發熱）',format1(ups),format1(ups/RT_PER_KW),format1(ups*3412.142)],['配電系統',format1(dist),format1(dist/RT_PER_KW),format1(dist*3412.142)],['照明設施',format1(light),format1(light/RT_PER_KW),format1(light*3412.142)],['人員',format1(people),format1(people/RT_PER_KW),format1(people*3412.142)],['合計',format1(totalHeatKw),format1(totalHeatKw/RT_PER_KW),format1(totalHeatKw*3412.142)]])}</section><section id='dc-power' class='dc-section'><h3>C. 預估用電容量 / NFB 估算</h3><table class='mobile-hide'><tr><th>項目</th><th>kW</th><th>A</th></tr>${[['IT + UPS 損耗',itUpsSupply],['空調總用電',totalCoolingPowerKw],['其他輔助用電',otherAuxPowerKw],['合計',tp]].map(([n,vv])=>`<tr><td>${n}</td><td>${format1(vv)}</td><td>${format1(cur(vv))}</td></tr>`).join('')}</table><h4>空調總用電組成</h4><table><tr><td>冰水主機</td><td>${format1(chillerKw)} kW</td></tr><tr><td>冷卻水塔</td><td>${format1(coolingTowerKw)} kW</td></tr><tr><td>冰水泵</td><td>${format1(chwPumpKw)} kW</td></tr><tr><td>冷卻水泵</td><td>${format1(cwPumpKw)} kW</td></tr><tr><td>空調箱 / 風機</td><td>${format1(ahuFanKw)} kW</td></tr></table></section><section id='dc-power-summary' class='dc-section'><h3>C-1. 總用電分析</h3>${renderMobileResultCards(['項目','數值'], [['IT + UPS 損耗',`${format1(itUpsSupply)} kW`],['空調總用電',`${format1(totalCoolingPowerKw)} kW`],['其他輔助用電',`${format1(otherAuxPowerKw)} kW`],['合計用電容量',`${format1(tp)} kW`],['合計電流',`約 ${format1(totalCurrentA)} A`]])}<table class='mobile-hide'><tr><td>IT + UPS 損耗</td><td>${format1(itUpsSupply)} kW</td></tr><tr><td>空調總用電</td><td>${format1(totalCoolingPowerKw)} kW</td></tr><tr><td>其他輔助用電</td><td>${format1(otherAuxPowerKw)} kW</td></tr><tr><td>合計用電容量</td><td>${format1(tp)} kW</td></tr><tr><td>合計電流</td><td>約 ${format1(totalCurrentA)} A</td></tr></table><p class='muted'>此處為供電容量初估，供 NFB / 幹線容量初步參考。</p></section><section id='dc-advice' class='dc-section'><h3>D. 建議配置</h3>${renderMobileResultCards(['項目','估算需求','建議配置'], [['UPS',`${format1(itUpsSupply)} kW`,`建議配置 ${format1(recommendedUpsKw)} kW`],['空調冷卻容量',`${format1(totalHeatKw)} kW / ${format1(totalHeatRt)} RT`,`建議配置 ${format1(recommendedCoolingRt)} RT`],['NFB / 幹線',`${format1(totalCurrentA)} A`,`建議配置 ${format1(recommendedNfbA)} A`]])}<table class='mobile-hide'><tr><th>項目</th><th>估算需求</th><th>建議配置</th></tr><tr><td>UPS</td><td>估算需求 ${format1(itUpsSupply)} kW</td><td>建議配置 ${format1(recommendedUpsKw)} kW</td></tr><tr><td>空調冷卻容量</td><td>估算需求 ${format1(totalHeatKw)} kW / ${format1(totalHeatRt)} RT</td><td>建議配置 ${format1(recommendedCoolingRt)} RT</td></tr><tr><td>NFB / 幹線</td><td>估算需求 ${format1(totalCurrentA)} A</td><td>建議配置 ${format1(recommendedNfbA)} A</td></tr></table><p class='muted'>實際配置仍需依備援架構、設備型錄、保護協調、啟動電流與法規確認。</p></section>`;const buildPieSection=(title,ariaLabel,parts,colors)=>{let acc=0;const totalForChart=Math.max(parts.reduce((sum,[,val])=>sum+val,0),1);const slices=parts.map((pt,idx)=>{const frac=pt[1]/totalForChart;const start=acc*360-90;acc+=frac;const end=acc*360-90;const largeArc=frac>0.5?1:0;const rad=(deg)=>deg*Math.PI/180;const x1=100+80*Math.cos(rad(start)),y1=100+80*Math.sin(rad(start)),x2=100+80*Math.cos(rad(end)),y2=100+80*Math.sin(rad(end));const d=`M 100 100 L ${x1} ${y1} A 80 80 0 ${largeArc} 1 ${x2} ${y2} Z`;return `<path d='${d}' fill='${colors[idx]}'></path>`;}).join('');const legend=parts.map(([n,val],idx)=>`<tr><td><span class='legend-dot' style='background:${colors[idx]}'></span>${n}</td><td>${format1(val)}</td><td>${format1((val/totalForChart)*100)}%</td></tr>`).join('');return `<h3>${title}</h3><div class='heat-pie-wrap'><svg class='heat-pie' viewBox='0 0 200 200' role='img' aria-label='${ariaLabel}'>${slices}</svg><table class='pie-legend'><tr><th>項目</th><th>kW</th><th>百分比</th></tr>${legend}</table></div>`;};const heatParts=[['IT 設備',it],['UPS 損耗',ups],['配電系統',dist],['照明設施',light],['人員',people]],heatColors=['#2563eb','#16a34a','#f59e0b','#a855f7','#ef4444'];const powerParts=[['IT + UPS 損耗',itUpsSupply],['空調總用電',totalCoolingPowerKw],['其他輔助用電',otherAuxPowerKw]],powerColors=['#0284c7','#0f766e','#b45309'];bars.innerHTML=`<section id='dc-chart' class='dc-section'>${buildPieSection('E. 散熱比例圖','散熱比例圓餅圖',heatParts,heatColors)}${buildPieSection('F. 總用電比例圖','總用電比例圓餅圖',powerParts,powerColors)}</section><section id='dc-pue' class='dc-section'><h3>G. 理論 PUE</h3><table class='mobile-hide'><tr><td>IT 負載</td><td>${format1(it)} kW</td></tr><tr><td>空調總用電</td><td>${format1(totalCoolingPowerKw)} kW</td></tr><tr><td>其他基礎設施用電</td><td>${format1(otherInfraKw)} kW</td></tr><tr><td>機房總用電初估</td><td>${format1(facilityTotalKw)} kW</td></tr><tr><td>理論 PUE</td><td>約 ${format1(pue)}</td></tr></table>${renderMobileResultCards(['項目','數值'], [['IT 負載',`${format1(it)} kW`],['空調總用電',`${format1(totalCoolingPowerKw)} kW`],['其他基礎設施用電',`${format1(otherInfraKw)} kW`],['機房總用電初估',`${format1(facilityTotalKw)} kW`],['理論 PUE',`約 ${format1(pue)}`]])}<p class='muted'>${pueHint}</p><p class='muted'>此為理論 PUE 初估，未納入備援架構、部分負載效率、季節 COP、控制策略與實測電表差異。</p></section>`;};['rw','rr','rk','rl','rwid','rh','unit','pp','uf','df','ld','or','vv','pf'].forEach(i=>document.getElementById(i).addEventListener('input',calc));}
