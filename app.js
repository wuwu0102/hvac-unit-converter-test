...
function initDcSharedTool(){const calc=()=>{const vals=['rw','rr','rk','rl','rwid','rh','pp','uf','df','ld','or','vv','pf'].map(x=>+document.getElementById(x).value);if(!vals.every(Number.isFinite)){rrr.textContent='-';return;}const [rows,per,kw,l,w,h,p,u,dfv,ldv,otherR,v,pff]=vals;const m=unit.value==='cm'?0.01:1,L=l*m,W=w*m,H=h*m,area=L*W,vol=area*H,tr=rows*per,it=tr*kw,ups=it*u,dist=it*dfv,light=area*ldv/1000,people=p*0.1,total=it+ups+dist+light+people,itUpsSupply=it+ups,hvac=total*0.4,other=it*otherR,tp=itUpsSupply+hvac+other,cur=(P)=>P*1000/(Math.sqrt(3)*v*pff),totalCurrentA=cur(tp),totalHeatRt=total/3.5168525,recommendedUpsKw=roundUpToStep(itUpsSupply,UPS_STEPS),recommendedCoolingRt=roundUpToMultiple(totalHeatRt,5),recommendedNfbA=roundUpToStep(totalCurrentA,NFB_STEPS);rrr.innerHTML=`<h3>A. 機房空間</h3><table><tr><td>排數</td><td>${format1(rows)}</td></tr><tr><td>每排機櫃數</td><td>${format1(per)}</td></tr><tr><td>總機櫃數</td><td>${format1(tr)}</td></tr><tr><td>每櫃 kW</td><td>${format1(kw)}</td></tr><tr><td>面積 m²</td><td>${format1(area)}</td></tr><tr><td>坪數</td><td>${format1(area/3.3058)}</td></tr><tr><td>體積 m³</td><td>${format1(vol)}</td></tr></table><h3>B. 散熱評估</h3><table><tr><th>項目</th><th>kW</th><th>RT</th><th>BTU/h</th></tr>${[['IT設備',it],['UPS（UPS 損耗 / 發熱）',ups],['配電系統',dist],['照明設施',light],['人員',people],['合計',total]].map(([n,vv])=>`<tr><td>${n}</td><td>${format1(vv)}</td><td>${format1(vv/3.5168525)}</td><td>${format1(vv*3412.142)}</td></tr>`).join('')}</table><h3>C. 預估用電容量 / NFB 估算</h3><table><tr><th>項目</th><th>kW</th><th>A</th></tr>${[['IT + UPS 損耗',itUpsSupply],['空調用電',hvac],['其他輔助用電',other],['合計',tp]].map(([n,vv])=>`<tr><td>${n}</td><td>${format1(vv)}</td><td>${format1(cur(vv))}</td></tr>`).join('')}</table><p class='muted'>IT + UPS 損耗為 UPS 供電容量初估，供 NFB / 幹線容量初估參考。</p><p class='muted'>此處為供電容量初估，實際 NFB 與幹線容量仍需依保護協調、啟動電流、備援架構、法規與設計條件確認。</p><h3>D. 建議配置</h3><table><tr><th>項目</th><th>估算需求</th><th>建議配置</th></tr><tr><td>UPS</td><td>${format1(itUpsSupply)} kW</td><td>${format1(recommendedUpsKw)} kW</td></tr><tr><td>空調冷卻容量</td><td>${format1(total)} kW / ${format1(totalHeatRt)} RT</td><td>${format1(recommendedCoolingRt)} RT</td></tr><tr><td>NFB / 幹線</td><td>${format1(totalCurrentA)} A</td><td>${format1(recommendedNfbA)} A</td></tr></table><p class='muted'>此處為容量級距初估。UPS 為供電容量，空調為冷卻容量，NFB / 幹線為電流容量；實際配置仍需依備援架構、設備型錄、保護協調、啟動電流與法規確認。</p>`;const parts=[['IT 設備',it],['UPS 損耗',ups],['配電系統',dist],['照明設施',light],['人員',people]],colors=['#2563eb','#16a34a','#f59e0b','#a855f7','#ef4444'];let acc=0;const totalForChart=Math.max(total,1);const slices=parts.map((pt,idx)=>{const frac=pt[1]/totalForChart;const start=acc*360-90;acc+=frac;const end=acc*360-90;const largeArc=frac>0.5?1:0;const rad=(deg)=>deg*Math.PI/180;const x1=100+80*Math.cos(rad(start)),y1=100+80*Math.sin(rad(start)),x2=100+80*Math.cos(rad(end)),y2=100+80*Math.sin(rad(end));const d=`M 100 100 L ${x1} ${y1} A 80 80 0 ${largeArc} 1 ${x2} ${y2} Z`;return `<path d='${d}' fill='${colors[idx]}'></path>`;}).join('');const legend=parts.map(([n,val],idx)=>`<tr><td><span class='legend-dot' style='background:${colors[idx]}'></span>${n}</td><td>${format1(val)}</td><td>${format1((val/totalForChart)*100)}%</td></tr>`).join('');bars.innerHTML=`<h3>E. 散熱比例圖</h3><div class='heat-pie-wrap'><svg class='heat-pie' viewBox='0 0 200 200' role='img' aria-label='散熱比例圓餅圖'>${slices}</svg><table class='pie-legend'><tr><th>項目</th><th>kW</th><th>百分比</th></tr>${legend}</table></div>`;};['rw','rr','rk','rl','rwid','rh','unit','pp','uf','df','ld','or','vv','pf'].forEach(i=>document.getElementById(i).addEventListener('input',calc));}
