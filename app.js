...
function initDcSharedTool(){const calc=()=>{const vals=['rw','rr','rk','rl','rwid','rh','pp','uf','df','ld','or','vv','pf'].map(x=>+document.getElementById(x).value);if(!vals.every(Number.isFinite)){rrr.textContent='-';return;}const [rows,per,kw,l,w,h,p,u,dfv,ldv,otherR,v,pff]=vals;const m=unit.value==='cm'?0.01:1,L=l*m,W=w*m,H=h*m,area=L*W,vol=area*H,tr=rows*per,it=tr*kw,ups=it*u,dist=it*dfv,light=area*ldv/1000,people=p*0.1,total=it+ups+dist+light+people,itUpsSupply=it+ups,hvac=total*0.4,other=it*otherR,tp=itUpsSupply+hvac+other,cur=(P)=>P*1000/(Math.sqrt(3)*v*pff);rrr.innerHTML=`<h3>A. 機房空間</h3><table><tr><td>排數</td><td>${format1(rows)}</td></tr><tr><td>每排機櫃數</td><td>${format1(per)}</td></tr><tr><td>總機櫃數</td><td>${format1(tr)}</td></tr><tr><td>每櫃 kW</td><td>${format1(kw)}</td></tr><tr><td>面積 m²</td><td>${format1(area)}</td></tr><tr><td>坪數</td><td>${format1(area/3.3058)}</td></tr><tr><td>體積 m³</td><td>${format1(vol)}</td></tr></table><h3>B. 散熱評估</h3><table><tr><th>項目</th><th>kW</th><th>RT</th><th>BTU/h</th></tr>${[['IT設備',it],['UPS（UPS 損耗 / 發熱）',ups],['配電系統',dist],['照明設施',light],['人員',people],['合計',total]].map(([n,vv])=>`<tr><td>${n}</td><td>${format1(vv)}</td><td>${format1(vv/3.5168525)}</td><td>${format1(vv*3412.142)}</td></tr>`).join('')}</table><h3>C. 預估用電容量 / NFB 估算</h3><table><tr><th>項目</th><th>kW</th><th>A</th></tr>${[['IT + UPS 損耗（供電容量估算，供 NFB / 幹線容量初估參考）',itUpsSupply],['空調',hvac],['其他輔助用電',other],['合計',tp]].map(([n,vv])=>`<tr><td>${n}</td><td>${format1(vv)}</td><td>${format1(cur(vv))}</td></tr>`).join('')}</table><p class='muted'>此處為供電容量初估，實際 NFB 與幹線容量仍需依保護協調、啟動電流、備援架構、法規與設計條件確認。</p>`;const parts=[['IT設備',it],['UPS損耗',ups],['配電',dist],['照明',light],['人員',people]];const mx=Math.max(...parts.map(pt=>pt[1]),1);bars.innerHTML=`<h3>D. 散熱比例圖</h3>${parts.map(([n,val])=>`<div class='bar'><span>${n}</span><i style='width:${(val/mx)*100}%'></i><em>${format1(val)}kW</em></div>`).join('')}`;};['rw','rr','rk','rl','rwid','rh','unit','pp','uf','df','ld','or','vv','pf'].forEach(i=>document.getElementById(i).addEventListener('input',calc));}
